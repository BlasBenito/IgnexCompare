...
SeqSlotBruteForceParallelTest=function(sequences=NULL, convergence.criteria=NULL, compute.p.value=NULL, cores=NULL, max.random.threshold=NULL, plot.title=NULL){
  ...
  if (is.null(convergence.criteria)){
    convergence.criteria=ceiling(10/cores)
  }

  p.value.iterations=trunc(10000/cores)
  ...
  results.foreach=foreach(dummy=1:cores, .combine=cbind) %dopar% SeqSlotBruteForceForParallel(sequences=sequences, convergence.criteria=convergence.criteria, compute.p.value=compute.p.value, starting.cost=starting.cost, max.random.threshold=max.random.threshold)
  ...
  parallel.slotting.solution$convergence.criteria=sum(unlist(results.foreach["convergence.criteria", ]))
...
#' @export
SeqSlotBruteForceForParallel=function(sequences, convergence.criteria, compute.p.value, dummy=dummy, starting.cost=starting.cost, max.random.threshold=max.random.threshold){
  ...
  while (convergence <= convergence.criteria){
    ...
    #if new.cost is equal to old.cost, we get closer to the convergence criteria
    if (new.cost == old.cost){
  ...
  sequences[[9]]=convergence.criteria
  ...
  names(sequences)=c(previous.names, "convergence.criteria", "lowest.costs", "best.slotting", "best.slotting.cost", "psi", "p.value")
  ...
  if (compute.p.value==TRUE){
    ...
    #p-value
    sequences$p.value=best.than/p.value.iterations
